...
class Visualization (object):
    ...
    def __init__(self, path, output_directory, study_title, first_condition_column, first_condition_order,
        ...
        self.x = np.transpose(self.df[columns_x].to_numpy())
        self.y = np.transpose(self.df[columns_y].to_numpy())

        columns_v = ['v_' + str(i) for i in range(self.NUM_TIMEPOINTS)]
        if all(col in self.df.columns for col in columns_v):
            self.v = np.transpose(self.df[columns_v].to_numpy())
        else:
            self.v = np.full((self.NUM_TIMEPOINTS, self.df.shape[0]), np.nan)

        columns_a = ['a_' + str(i) for i in range(self.NUM_TIMEPOINTS)]
        if all(col in self.df.columns for col in columns_a):
            self.a = np.transpose(self.df[columns_a].to_numpy())
        else:
            self.a = np.full((self.NUM_TIMEPOINTS, self.df.shape[0]), np.nan)
        v_df = self.df[columns_v]
        a_df = self.df[columns_a]
        self.v = np.transpose(v_df.to_numpy())
        self.a = np.transpose(a_df.to_numpy())
    ...
    def plot_kinematics(self, metric='velocity'):
        prefix = 'v_' if metric == 'velocity' else 'a_'
        relevant_cols = [col for col in self.df.columns if col.startswith(prefix)]

        data_matrix = self.v if metric == 'velocity' else self.a

        # בדיקה קריטית: אם העמודות לא קיימות או שכל הערכים בהן הם NaN (קורה כשאין RT)
        if not relevant_cols or np.isnan(data_matrix).all():
            print(f"Skipping {metric} plot: No valid kinematic data found in the dataset.")
            return
        ...
        if isinstance(self.colormap, list):
            colors = self._normalize_colors(
                self.colormap[:num_colors_needed] if len(self.colormap) > num_colors_needed else self.colormap)
        else:
            colors = plt.cm.get_cmap(self.colormap)(np.linspace(0, 1, num_colors_needed))
        ...
        for i in range(len(self.conditions_2)):
            ...
            for j in range(len(self.conditions_1)):
                ...
                data = data_matrix[:, cur_ind]
                valid_rows = ~np.isnan(data).all(axis=0)
                if valid_rows.sum() == 0:
                    continue
                ...
                se_data = std_data / np.sqrt(self.num_subjects)

                label = self._get_display_label(cond_1,
                                                self.first_condition_column) if self.first_condition_column else str(
                    cond_1)
                time_steps = np.arange(self.NUM_TIMEPOINTS)

                ax.plot(time_steps, mean_data, c=colors[color_idx], label=label, linewidth=2)
                ax.fill_between(time_steps, mean_data - se_data, mean_data + se_data, color=colors[color_idx],
                                alpha=0.2)
                ax.tick_params(labelsize=self.ticks_size)
                color_idx += 1
            ...
            if self.first_condition_column:
                ax.legend(fontsize=self.legend_size)
            ax.set_xlabel('Time Step (Normalized)', fontsize=self.labels_size)
            ylabel = 'Velocity (pixels/ms)' if metric == 'velocity' else 'Acceleration (pixels/ms^2)'
            ax.set_ylabel(ylabel, fontsize=self.labels_size)
            title_prefix = 'Mean Velocity' if metric == 'velocity' else 'Mean Acceleration'
            if self.second_condition_column:
                ax.set_title(f'{title_prefix} - {cond_2}', fontsize=self.title_size)
            else:
                ax.set_title(title_prefix, fontsize=self.title_size)

        plt.savefig(self.output_directory + os.sep + f'Average {metric}', dpi=300)
        plt.show()
